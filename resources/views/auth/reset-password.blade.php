@extends('layout.app', [
    'includeHeader' => true,
    'pageTitle' => __('auth.pageTitle.reset-password'),
])
...
@section('body')
    <div class="max-w-full mx-2 mb-4 flex-1">
        ...
        <section class="max-w-md mx-auto text-center">
            <header>
                <h1 class="section-header mb-2">{{ __('auth.headline.reset-password') }}</h1>
                <p class="mb-2 text-sm md:text-base">{{ __('auth.intro.reset-password') }}</p>
            </header>
            ...
            <form action="{{ route('password.update') }}" method="POST">
                ...
                <div class="">
                    <div class="mb-2 text-left">
                        ...
                        <input readonly id="email" name="email" placeholder="{{ __('auth.email') }}" type="text" class="@error('email') error @enderror w-full" value="{{ old('email', request()->get('email')) }}">
                        @error('email')
                    ...
                    <div class="flex items-baseline justify-between">
                        <button type="submit" class="icon-button mt-4">
                            <span>{{__('auth.buttonResetPassword')}}</span>
